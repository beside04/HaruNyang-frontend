...
//       Get.arguments == null ? selectedIndex.value = 0 : selectedIndex.value = Get.arguments['index'];
//
//       Get.find<OnBoardingController>().state.value.age == '' || Get.find<OnBoardingController>().state.value.age == null ? null : await goToBirthPage();
//     });
//   }
//
...
//       int timeDifference = now.difference(DateTime.parse(lastBirthDayPopupDate!)).inDays;
//
//       if (timeDifference < 1) {
...
//
//     final birthday = dateFormat.parse('${Get.find<OnBoardingController>().state.value.age}');
//
//     final nowBirthday = DateTime(now.year, birthday.month, birthday.day);
//
//     await getLastBirthDayPopUpDate();
//
//     if (isNotBirthDayPopup.value && dateFormat.format(nowBirthday) == dateFormat.format(now)) {
//       await Get.to(
//         () => BirthDayScreen(
